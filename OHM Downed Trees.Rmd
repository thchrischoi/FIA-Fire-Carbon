...
```{r}
## point to TLS data
files.trunk <- list.files("Data/TLS_Scans/Trees/All_Downed_Trees", pattern="trunk_downed_upright.csv", full.names = TRUE, recursive=TRUE)

files.branch <- list.files("Data/TLS_Scans/Trees/All_Downed_Trees", pattern="branch_downed.csv", full.names = TRUE, recursive=TRUE)
...
Output data
```{r}
write.csv(TLS.carbon, 'Data/TLS_Carbon_Downed_20251026.csv')
